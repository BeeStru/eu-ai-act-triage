...
**High-risk (Article 6).** Systems permitted but subject to the heaviest requirements. Two routes in: Annex I, where the AI is a safety component of, or is itself, a product already regulated under EU harmonisation legislation (medical devices, lifts, radio equipment); and Annex III, a closed list of eight use-case areas (biometrics, critical infrastructure, education, employment, essential services, law enforcement, migration, justice and democratic processes). Most workplace AI questions land in the Annex III employment area.
...
The Digital Omnibus on AI amends the Act's timeline, most significantly postponing the high-risk obligations for stand-alone Annex III systems from 2 August 2026 to 2 December 2027, and for Annex I systems to 2 August 2028. It also postpones the Article 50(2) marking duty to 2 December 2026 and adds the new Article 5 prohibition. As of the last verification date recorded in the data files, the Omnibus has passed the European Parliament (16 June 2026) and the Council (29 June 2026) and was signed on 8 July 2026, but has not been published in the Official Journal. Until publication and entry into force, the original dates remain the binding law and the amended dates are the operative planning baseline.

Both facts are true at once, and a tool that showed only one would mislead. Reporting only the original dates would ignore the near-certain amendment that every organisation is planning against. Reporting only the amended dates would state as law something that is not yet law. So every obligation carries both dates and a status flag (in application, unchanged, postponed by the Omnibus, or introduced by the Omnibus), and the output states the amendment's legal status in full. When the Omnibus is published, the correction is a single edit to one data file.
...
All legal content is held in the package data directory (`src/aiact_triage/data/`) as JSON, separate from the engine, with an article citation on every rule and a metadata block recording sources; the rule set's verification date is recorded in the timeline data and carried into every audit record. Encoding is drawn from Regulation (EU) 2024/1689 as published in the Official Journal, and, for the amendments, from the Council of the EU press release of 7 May 2026 and the provisionally agreed compromise text in Council document 9247/26.
...
The amended dates follow the compromise text and the institutions' announcements. They are re-verified against the Official Journal text on publication, expected in July 2026, and any correction is committed with a reference to the Official Journal citation. Because the legal content sits in versioned data files, that correction is a single-file change, and the commit that makes it is itself a record that the law was tracked to source.

## 9. Known limits

The Annex I route is handled at the level of "product under Annex I harmonisation legislation" rather than per instrument, which is adequate for a deployer triage but coarser than a specialist product-safety assessment would need. The law-enforcement exceptions inside the real-time biometric identification prohibition are surfaced in the intake question rather than modelled individually. The new prohibition's application date rests on secondary reporting of the compromise text pending Official Journal publication.
