...
All legal content sits in `src/aiact_triage/data/*.json`, separate from the engine, with an article citation on every rule and a metadata block recording sources; the rule set's verification date is recorded in the timeline data and carried into every audit record. Encoded from Regulation (EU) 2024/1689 as published in the Official Journal and, for the amendments, the Council press release of 7 May 2026 and the provisionally agreed compromise text (Council document 9247/26). When the Omnibus appears in the Official Journal, any date correction is a one-file change, and the tests that assert dates read from the same file.
